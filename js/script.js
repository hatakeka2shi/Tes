...
function tampilkanArtikel() {
  ...
  if (wrapper) {
    const daftarArtikel = [
      ...
      {
        judul: "Lomba Video Kreasi",
        gambar: "img/artikel/artikel_2.webp",
      ...
      {
        judul: "Lomba Karate",
        gambar: "img/artikel/artikel_3.webp",
    ...
    wrapper.innerHTML = dataTampil.map(item => `
      <div class="artikel-card">
        ...
        <div class="artikel-content">
          <h3>${item.judul}</h3>
          <p>${item.ringkasan}</p>
          <a href="${item.link}" target="_blank" class="read-more">Baca Selengkapnya</a>
        </div>
      </div>
    `).join('');
  }
